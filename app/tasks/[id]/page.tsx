...
export default function TaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        className={`
        ${isExpired(task.dueDate) ? "border-red-500 dark:border-red-700" : ""}
        ${isNearDeadline(task.dueDate) && !isExpired(task.dueDate) ? "border-amber-500 dark:border-amber-700" : ""}
      `}
      >
        <CardHeader>
      ...
      <TaskDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} task={task} />
    </div>
  );
}
